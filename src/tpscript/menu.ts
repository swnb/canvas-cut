...
export class Menu extends Draw {
    ...
    drawImgById() {
        ...
        this.ListOfObjs.forEach(
            (obj: { name: string; prefix: number }, index: number) => {
                ...
                this.context.fillText(this.ListOfnames[index], this.x, this.y + index * divi + obj.prefix + ImgElement.height + 30)
            }
        );
    }
    init() {
    ...
    drawTriangleObj(): [number, number, number] {
        ...
        const t1 = getImg("t1");
        this.drawImg(t1, this.x - len - t1.width / 2, this.y, t1.width, t1.height);
        this.context.fillText("等腰直角三角形", this.x - len, this.y * 3 - 20);
        const t2 = getImg("t2");
        this.drawImg(t2, this.x - len - t2.width / 2, this.y * 3, t2.width, t2.height);
        this.context.fillText("等腰三角形", this.x - len, this.y * 4.8 - 20);
        const t3 = getImg("t3");
        this.drawImg(t3, this.x - len - t3.width / 2, this.y * 4.8, t3.width, t3.height);
        this.context.fillText("等边三角形", this.x - len, this.y * 7.2 - 20);
        const t4 = getImg("t4");
        this.drawImg(t4, this.x - len - t4.width / 2, this.y * 7.2, t4.width, t4.height);
        this.context.fillText("钝角三角形", this.x - len, this.y * 9.5 - 20);
        const t5 = getImg("t5");
        this.drawImg(t5, this.x - len - t5.width / 2, this.y * 9.5, t5.width, t5.height);
        this.context.fillText("直角三角形", this.x - len, this.y * 11.5 - 15);
        const t6 = getImg("t6");
        this.drawImg(t6, this.x - len - t6.width / 2, this.y * 11.5, t6.width, t6.height);
    ...
    drawParallelogramObj(): [number, number, number] {
        ...
        this.context.font = "15px sans-serif";
        const p1 = getImg("p1");
        this.drawImg(p1, this.x - len - p1.width / 2, this.y, p1.width, p1.height);
        this.context.fillText("平行四边形A", this.x - len, this.y * 3 - 20);
        const p2 = getImg("p2");
        this.drawImg(p2, this.x - len - p2.width / 2, this.y * 3, p2.width, p2.height);
        this.context.fillText("平行四边形B", this.x - len, this.y * 5 - 20);
        const p3 = getImg("p3");
        this.drawImg(p3, this.x - len - p3.width / 2, this.y * 5, p3.width, p3.height);
        this.context.fillText("平行四边形C", this.x - len, this.y * 8 - 20);
        const p4 = getImg("p4");
        this.drawImg(p4, this.x - len - p4.width / 2, this.y * 8, p4.width, p4.height);
        this.context.fillText("长方形", this.x - len, this.y * 10 - 20);
        const p5 = getImg("p5");
        this.drawImg(p5, this.x - len - p5.width / 2, this.y * 10, p5.width, p5.height);
    ...
    drawIrregularObj(): [number, number, number] {
        ...
        this.context.font = "15px sans-serif";
        const I1 = getImg("I1");
        this.drawImg(I1, this.x - len - I1.width / 2, this.y + 425, I1.width, I1.height);
        this.context.fillText("直角梯形", this.x - len, this.y + 515);
        const I2 = getImg("I2");
        this.drawImg(I2, this.x - len - I2.width / 2, this.y + 535, I2.width, I2.height);
