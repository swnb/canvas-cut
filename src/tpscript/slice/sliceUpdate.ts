	...
	const insertPoints = lines.map(
		(line): InsertPoint | undefined => {
			switch (line.type) {
				...
				case "curve": {
					const [firstPoints, secondPoints, bezier, midPoint] = line.points;

					const r = line.r;

					// 对点进行处理
					const resSector = util.getInsCircle(lineA1, lineA2, midPoint, r);
...
const SC = (
	...
	switch (firstInsert.insertP.type) {
		case "insertPoint-line": {
			...
			const closeLine: Straight = {
				type: "line",
				points: [secondInsert.insertP.points[0], firstInsert.insertP.points[0]]
			};
